document.addEventListener("DOMContentLoaded", () => {
  ...
  function generateSemesterCards(count, mallaData = null) {
    ...
    for (let i = 1; i <= count; i++) {
                    ...
                    <div class="top">
                        ...
                        <div class="mas">
                            <button class="add-ramo-btn primary" data-semester="${i}">
                                <i class='bx bx-plus'></i> <span class="agregar">Agregar Ramo</span>
                            </button>
                        </div>
                    </div>
      ...
      if (mallaData) {
        const ramosDelSemestre = mallaData.semesters.find(
          (s) => s.number === i
        ).ramos;
  ...
  function addRamoToSemester(
    ...
    const prerequisitePendingClass =
      prerequisite && !isCompleted ? "prerequisite-pending" : "";
  ...
  semestersGrid.addEventListener("click", (event) => {
      ...
      if (ramoCard) {
        if (!ramoCard.classList.contains("completed")) {
          ...
          if (canBeMarked) {
            ...
            alert(
              "No puedes marcar este ramo como completado. ¡Primero debes completar su prerrequisito!"
            );
          }
        } else {
          ...
          if (!hasDependents) {
            ...
            alert(
              "No puedes desmarcar este ramo. Debes desmarcar los ramos que dependen de él primero."
            );
          }
        }
      }
    ...
    if (editRamoBtn) {
      ...
      prerequisiteSelect.value =
        currentRamoCard.dataset.ramoPrerequisitos || "";
  ...
  saveEditBtn.addEventListener("click", () => {
    ...
    if (newName.trim() !== "") {
      ...
      currentRamoCard.querySelector(".edit-ramo-btn").dataset.ramoName =
        newName;
    }
  ...
  deleteRamoBtn.addEventListener("click", () => {
    if (
      confirm(
        "¿Estás seguro de que quieres eliminar este ramo? Esta acción es irreversible."
      )
    ) {
      if (currentRamoCard) {
  ...
  function checkDependents(ramoCard) {
    ...
    allRamoCards.forEach((card) => {
      ...
      if (
        prerequisiteOfCard === currentRamoName &&
        card.classList.contains("completed")
      ) {
